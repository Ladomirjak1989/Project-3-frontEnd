...
function FlightElAdmin({ flight, isCart }) {
        ...
        <li className='mb-4 p-6 bg-white rounded-lg shadow-lg w-full'>
            <div className="p-4 bg-gray-100 rounded-md shadow-md w-full">
                ...
                <div className="mt-4 flex flex-col md:flex-row md:justify-between items-center">
                    <Link to={`/${currentLang}/flights-admin/${flight._id}`} className="text-blue-500 hover:underline">SHOW MORE...</Link>
                    
                    <div className="flex space-x-5 mt-2 md:mt-0">
                        {user?.role === "admin" && (
                            <>
                                <Link to={`/${currentLang}/flights/admin-updated/${flight._id}`} className="text-blue-500 hover:underline">EDIT</Link>
