...
import { deleteHotelAsync } from '../../Store/Slices/fetchHotelSliceAsync';
...
function HotelEl({ _id, name, images, location, rooms, rating, board, randomReviews, isFavorite, isCart, type }) {
    ...
    const handleDeleteHotel = async (id) => {
        try {
            const deleted = await dispatch(deleteHotelAsync(id));
            if (deleted) {
                dispatch(setPopUp("Hotel successfully deleted"));
            }
        } catch (error) {
